...
% Plots x and y variables with x and y errorbars % usage:
% errorbarxy(x,y,lx,ly,ux,uy,linecol,errorcol)
% lx,ly are lower
% error bounds, must be same size as x,y 
% ux uy are upper bounds
% (optional, if omitted, errorbarsxy uses lx,ly on both sides)
% linecol: Propertes for data, same as for the plot function
% errorcol: Propertes for data, same as for the plot function
% to use properties:
% usage:
% errorbarxy(x,y,lx,ly,[],[],linecol,errorcol) %James Rooney, 17
% October 2003 
...
ax = axis;
xw=(max(ax(1:2))-min(ax(1:2)))/200;
yw=(max(ax(3:4))-min(ax(3:4)))/200;
