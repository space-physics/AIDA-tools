function [az,ze] = inv_project_directions(px,py,inimg,r,optmod,optpar,e_n,l_0,cmtr)
% INV_PROJECT_DIRECTIONS - pixels line-of-sight to azimuth and zenith angles
% 
...
epix = rotmtr*epix;
epix = epix';
epix(imag(epix(:,1))~=0,:) = nan;
epix(imag(epix(:,2))~=0,:) = nan;
epix(imag(epix(:,3))~=0,:) = nan;

% Calculate the zenith and azimuthal angles:
ze = acos(epix(:,3));
az = atan2(epix(:,1),epix(:,2));
